...
if test ! "$cmd"; then
    ...
    echo "  test.coverage [N]   run tests with coverage report (default threshold: 50%)"
    echo "  test.coverage.show  show coverage report without threshold check"
    echo "  test.integration    run integration tests against live WoWInterface data"
    echo "  test.verbose        run tests with verbose output"
    echo "  update              scrape data and generate catalogues"
    echo "  validate <path>     validate a catalogue JSON file"
    exit 1
fi
...
if test "$cmd" = "build"; then
    ...
    tbd=(
        ...
        # Note: cache directory is NOT cleaned - it contains valuable HTTP responses
        "*.json" # output files
    )
...
elif test "$cmd" = "test.integration"; then
    echo "Running integration tests against live WoWInterface data..."
    echo "This will make real HTTP requests and may take a while."
    echo
    # Run integration tests with verbose output
    go test -tags=integration -v ./src/wowi/
    exit 0
    ...
    echo "Scraping WoWInterface and GitHub data..."
    go run . scrape --source wowinterface --source github

    echo
    echo "Update complete. Generated catalogues:"
    ls -lh state/*.json 2>/dev/null || echo "No catalogue files found in state/"
    exit 0

elif test "$cmd" = "validate"; then
    if test -z "$rest"; then
        echo "Error: validate command requires a catalogue file path"
        echo "Usage: ./manage.sh validate <path/to/catalogue.json>"
        exit 1
    fi
